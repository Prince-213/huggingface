...
export async function POST(request: Request) {
  ...
  for await (const chunk of hf.chatCompletionStream({
    ...
    messages: [
      {
        role: "user",
        content: `you are a school chat bot called Godfrey okoye university also known as go uni, greet and respond as such. from these info concerning the school ${gouniInfo}. give answers to the question i ask. let your answer be as short as possible. your question is ${question.prompt}`,
      },
    ],
    max_tokens: 500,
    temperature: 0.1,
